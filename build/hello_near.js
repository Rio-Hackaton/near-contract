...
class Property {
  constructor(id, type, cep, latitude, longitude, imageUrl, owner) {
    this.id = id;
    this.type = type;
    this.cep = cep;
...
let RealEstateContract = (_dec = NearBindgen({
  requireInit: true
}), _dec2 = initialize(), _dec3 = call({}), _dec4 = view(), _dec5 = view(), _dec6 = call({
  privateFunction: true
}), _dec(_class = (_class2 = class RealEstateContract {
  properties = new UnorderedMap('properties');
  admin = "paulopaula.testnet"; // Campo para armazenar o administrador do contrato
  ...
  addProperty({
    ...
    log(`Imóvel adicionado com ID: ${id}`);
    return id;
  }
  getPropertiesByWallet({
    walletId
  }) {
    ...
    const allKeys = this.properties.keys({
      start: 0,
      limit: 10000
    }); // Exemplo usando limites arbitrários para demonstração
    const allProperties = [];
  ...
  changeAdmin({
    ...
    assert(predecessorAccountId() === this.admin, "Somente o administrador pode mudar o administrador.");
    this.admin = newAdmin;
  }
}, (_applyDecoratedDescriptor(_class2.prototype, "init", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "init"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "addProperty", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "addProperty"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getPropertiesByWallet", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "getPropertiesByWallet"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getPropertyById", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "getPropertyById"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "changeAdmin", [_dec6], Object.getOwnPropertyDescriptor(_class2.prototype, "changeAdmin"), _class2.prototype)), _class2)) || _class);
function changeAdmin() {
  const _state = RealEstateContract._getState();
  if (!_state && RealEstateContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = RealEstateContract._create();
  if (_state) {
    RealEstateContract._reconstruct(_contract, _state);
  }
  const _args = RealEstateContract._getArgs();
  const _result = _contract.changeAdmin(_args);
  RealEstateContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(RealEstateContract._serialize(_result, true));
}
...
function getPropertiesByWallet() {
  const _state = RealEstateContract._getState();
  if (!_state && RealEstateContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = RealEstateContract._getArgs();
  const _result = _contract.getPropertiesByWallet(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(RealEstateContract._serialize(_result, true));
}
...
export { addProperty, changeAdmin, getPropertiesByWallet, getPropertyById, init };
